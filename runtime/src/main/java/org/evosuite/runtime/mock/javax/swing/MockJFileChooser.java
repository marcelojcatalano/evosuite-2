...
public class MockJFileChooser extends javax.swing.JFileChooser implements OverrideMock {
    ...
    public MockJFileChooser() {
        // inicializar VFS antes de cualquier uso
        VirtualFileSystem virtualFileSystem = VirtualFileSystem.getInstance();
        virtualFileSystem.init();

        MockFileSystemView mockFileSystemView = (MockFileSystemView) MockFileSystemView.getFileSystemView();

        // obtener el directorio de trabajo del VFS (siempre definido)
        String vfsRootPath = VirtualFileSystem.getWorkingDirPath();

        this.setFileSystemView(mockFileSystemView);

        File rootAsFile = new File(vfsRootPath); // <-- esta es la parte correcta t
        File defaultDir = mockFileSystemView.getDefaultDirectory();
        if (defaultDir == null) {
            try {
                defaultDir = mockFileSystemView.createNewFolder(rootAsFile);
            } catch (IOException e) {
                throw new RuntimeException("No se pudo creaar el Directorio");
            }
        }
    ...
    public MockJFileChooser(File currentDirectory, FileSystemView fsv) {
        ...
        // Forzar siempre el MockFileSystemView para evitar FileSystemView de Swing.
        if (!(fsv instanceof MockFileSystemView)) {
            fsv = new MockFileSystemView() {
                @Override
                public File createNewFolder(File containingDir) throws IOException {
                    // EvoSuite nunca usa esto durante test generation,
                    // así que es seguro devolver una carpeta mock.
                    MockFile dir = new MockFile(
                            new File(containingDir, "newFolder").getPath()
                    );
                    dir.mkdirs();
                    return dir;
                }
        ...
        // --- Directorio actual determinista y compatible con VFS ---
        File dirToUse;

        if (currentDirectory == null) {
            // "." es la opción más estable; se resuelve dentro del VFS de EvoSuite
            dirToUse = new MockFile(".");
            if (!dirToUse.exists()) {
                dirToUse.mkdirs();
            }
        } else {
            // Si viene un File pero no es un MockFile, convertirlo
            if (!(currentDirectory instanceof MockFile)) {
                dirToUse = new MockFile(currentDirectory.toURI());
            } else {
                dirToUse = currentDirectory;
            }
        }

        setCurrentDirectory(dirToUse);
    }
    ...
    private void installShowFilesListener() {
        ...
        // Avoid touching real AWT when running with VFS (mock environment)
        if (RuntimeSettings.useVFS) {
            return;
        }

        // Avoid touching Toolkit in headless environments (WSL, CI, etc.)
    ...
    public File getCurrentDirectory() {
        File cd = super.getCurrentDirectory();
        if (cd == null) {
            // fallback estable, mismo que el constructor
            cd = new MockFile(".");
            cd.mkdirs();
            super.setCurrentDirectory(cd);
        }
        return cd;
    }
    ...
    public void setFileSystemView(FileSystemView fsv) {
        ...
        if (fsv == null) {
            fileSystemView = MockFileSystemView.getFileSystemView();
        } else {
            fileSystemView = fsv;
        }
    ...
    public void updateUI() {
        ...
        if (fileSystemView == null) {
            // Don't acquire the system FileSystemView which may touch Toolkit/OS.
            // Use our MockFileSystemView as a safe deterministic default.
